...
def parse_args():
    ...
    parser.add_argument('--out-dir', type=str, default='segmentation/new_underseg_overseg/segmentation_output')
...
def read_results(method, res_dirs, id_):
    ...
    if method == 'segmenter':
        ...
        segs, scores = convert_dets_to_segs(pts_velo_cs, velo_to_cam_tf, dets)
    return segs, scores
...
for id_ in ids:
    ...
    for i in range(len(gtsegs)):
        for j in range(i+1, len(gtsegs)):
            intersected_inds = np.intersect1d(gtsegs[i], gtsegs[j])
            if len(intersected_inds) > 0:
                ambiguous_inds = np.union1d(ambiguous_inds, intersected_inds)
    ...
    #
    for i in range(len(segs)):
        segs[i] = np.setdiff1d(segs[i], ambiguous_inds)
    
    for i in range(len(gtsegs)):
        gtsegs[i] = np.setdiff1d(gtsegs[i], ambiguous_inds)
    ...
    for i in range(len(gtsegs)):
        ...
        num_gt_overlap += gt_has_ovlp[i]

        # this writes ovlp flag
        ...
        # this does not write ovlp flag
        eval_fd.write('%d %s -1 %f %f -1 %f -1 -1 %f -1 -1 %f -1 %f -1 -1 0\n' % (
            id_, gtclasses[i], pos_points, blob_points, other_pos_points, gt_dist, 0, -1
        ))

    eval_fd.close()

print('total number of gt:', num_gt_total)
print('total number of gt with 0 points:', num_gt_zero_point)
print('total number of gt with overlap:', num_gt_overlap)
